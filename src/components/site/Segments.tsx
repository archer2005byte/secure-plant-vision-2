...
function Capability({ icon: Icon, label }: { icon: typeof Eye; label: string }) {
  return (
    <div className="flex min-w-0 items-center gap-2 px-3 py-2 text-white">
...
function EstablishedVisual() {
  return (
    <div className="relative h-[272px] overflow-hidden bg-[#183047] text-white">
      <img
      ...
      <div className="absolute inset-x-0 bottom-0 grid grid-cols-4 divide-x divide-white/20 bg-[#10202D]/96">
...
function BrownfieldVisual() {
  return (
    <div className="relative h-[272px] overflow-hidden bg-[#645E55] text-white">
      ...
      <div className="absolute right-[28%] top-[116px] flex items-center">
        <div className="h-1 w-12 bg-ey-yellow" />
        <span className="h-0 w-0 border-y-[11px] border-l-[17px] border-y-transparent border-l-ey-yellow" />
      </div>

      <div className="absolute bottom-4 right-4 top-5 w-[29%] overflow-hidden rounded-md border border-ey-yellow bg-[#FFFDF3]/96 text-[#1A1A24] shadow-lg">
        <div className="grid h-full grid-cols-2 gap-x-2 gap-y-2 px-3 py-3">
    ...
    <div className="relative mt-2 grid grid-cols-4 gap-1 rounded-md bg-[#FFF9D8] px-2 pb-2 pt-2">
      ...
      {stages.map((stage) => (
        <div key={stage} className="relative z-[1] flex min-w-0 flex-col items-center text-center">
          ...
          <span className="mt-2 text-[11px] font-bold uppercase leading-[1.05] text-[#1A1A24]">{stage}</span>
        </div>
      ))}
    </div>
  );
...
export function Segments() {
  return (
    <section id="segments" className="scroll-mt-24 bg-background px-5 py-4 text-foreground md:px-8" style={{ minHeight: "calc(100vh - 98px)" }}>
      <div className="mx-auto w-full max-w-6xl">
            ...
            <article key={pathway.title} className={index === 1 ? "border-t border-[#C9C9D0] lg:border-l lg:border-t-0" : ""}>
              <div className="px-5 py-3">
                <div className="flex items-start gap-3">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-ey-yellow text-[22px] font-bold text-[#1A1A24]">{pathway.number}</span>
                  <div className="min-w-0">
                    <h3 className="text-[20px] font-semibold leading-[1.08] text-[#1A1A24]">{pathway.title}</h3>
                    <p className="mt-1 text-[11px] font-bold uppercase tracking-[0.11em] text-[#5A5A66]">{pathway.strap}</p>
                  </div>
                </div>
                ...
                <div className="mt-3 flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[13px] font-bold text-[#1A1A24]">Where they are today</p>
                    <ul className="mt-1 space-y-0.5 text-[11.5px] leading-[1.18] text-[#353541]">
                      {pathway.bullets.map((bullet) => (
                        <li key={bullet} className="flex gap-2"><span className="mt-[5px] h-1 w-1 shrink-0 rounded-full bg-[#1A1A24]" /><span>{bullet}</span></li>
                      ))}
                    </ul>
                  </div>
                </div>
                ...
                <div className="mt-3 border-t border-[#DEDEE3] pt-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-[13px] font-bold text-[#1A1A24]">Modernisation pathway</p>
                  </div>
                ...
                <div className="mt-2.5 flex gap-3 border-t border-[#DEDEE3] pt-2.5">
                  ...
                  <div>
                    <p className="text-[13px] font-bold text-[#1A1A24]">Target state</p>
                    <p className="mt-0.5 text-[11.5px] leading-[1.18] text-[#353541]">{pathway.target}</p>
                  </div>
                </div>
              </div>
            </article>
        ...
        <div className="mt-3 flex items-center gap-3 rounded-lg border border-[#E2C900] bg-[#FFFDF0] px-4 py-2.5 text-[#1A1A24]">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-semibold leading-[1.2]">Plant type defines the starting context. Asset criticality, geography and threat exposure then determine the required security posture.</p>
            <p className="mt-0.5 text-[11px] leading-[1.2] text-[#5A5A66]">The following sections translate this into asset-level security requirements, assessment priorities and operational use cases.</p>
          </div>
        </div>
      </div>
    </section>
